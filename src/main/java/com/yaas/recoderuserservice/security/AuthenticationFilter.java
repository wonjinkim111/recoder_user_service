...
public class AuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        String token = Jwts.builder()
        		.setSubject(String.valueOf(usersDto.getUserId()))
        	    .setExpiration(new Date(System.currentTimeMillis() + Long.parseLong(this.env.getProperty("token.expiration_time"))))
        	    .signWith(SignatureAlgorithm.HS512, this.env.getProperty("token.secret")).compact();        	    
        response.addHeader("token", token);
        response.addHeader("userId", String.valueOf(usersDto.getUserId()));
    }
}
